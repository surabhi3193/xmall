package com.mindinfo.xchangemall.xchangemall.webrtc.activities;
...
import com.mindinfo.xchangemall.xchangemall.webrtc.classes.adapters.OpponentsFromCallAdapter;
...
public class VideoConversationFragment  extends BaseConversationFragment implements Serializable, QBRTCClientVideoTracksCallbacks<QBRTCSession>,
    ...
    private SurfaceView remoteFullScreenVideoView;
    ...
    protected void configureOutgoingScreen() {
        outgoingOpponentsRelativeLayout.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.dark_trans));
    ...
    protected void configureToolbar() {
        ...
        toolbar.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.dark_trans));
...
//        remoteFullScreenVideoView.(localViewOnClickListener);
...
//      get mainVideoTrack - opponent's video track from full screen
        ...
        SurfaceView remoteVideoView = findHolder(userId).getOpponentView();
...
//        final SurfaceView remoteVideoView = itemHolder.getOpponentView();
//
...
//            childViewHolder.getOpponentView().release();
     ...
     */
    private void fillVideoView(int userId, SurfaceView videoView, QBRTCVideoTrack videoTrack,
                               boolean remoteRenderer) {
        videoTrack.removeRenderer(videoTrack.getRenderer());
        videoTrack.addRenderer(new VideoRenderer((VideoRenderer.Callbacks) videoView));
        if (userId != 0) {
...
//            updateVideoView(videoView, isCurrentCameraFront);
    ...
    private void fillVideoView(SurfaceView videoView, QBRTCVideoTrack videoTrack, boolean remoteRenderer) {
